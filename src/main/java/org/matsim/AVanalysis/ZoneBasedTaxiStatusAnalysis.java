...
/**
 * @author teoal customizing jbischoff
 *
 */
...
public class ZoneBasedTaxiStatusAnalysis implements LinkEnterEventHandler, ActivityEndEventHandler {
	...
	// n.B. A taxi request set off at 14:50, arriving at customer at 15:01 would be counted to happen at 14:00 hours
	...
	public void handleEvent(ActivityEndEvent event) {
		if (event.getActType().startsWith("TaxiStay")) { //per drt analysis potrebbe essere solo "Stay"
			handleStayEvent(event);
		} else if (event.getActType().startsWith("TaxiPickup")) { //per drt analysis potrebbe essere "PassengerPickup"
			handlePickupEvent(event);
		} else if (event.getActType().startsWith("BeforeVrpSchedule")) { //per drt analysis potrebbe essere solo "Before schedule"
			handleBeforeSchedule(event);
		} else if (event.getActType().startsWith("TaxiDropoff")) { //per drt analysis potrebbe essere solo "PassengerDropoff"
