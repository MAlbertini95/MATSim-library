...
public class BasicDemandGenerator {
	...
	//La struttura dei file è generalmente importante, vedi la cartella BasicDemandTutorial per approfondimenti
	private static final String NETWORKFILE = "Path to Network/network.xml";
	private static final String REGION = "Path to /Region.shp";
	
	private static final String PLANSFILEOUTPUT = "Path to output plans file/plans.xml";
	
	private static final String SHOPS = "Path to shops/shops.txt";
	private static final String KINDERGARTEN = "Path to infant school/kindergaerten.txt";
	...
	private void run() {
		this.shapeMap = readShapeFile(REGION, "Nr");
...
//				System.out.println(this.findClosestCoordFromMap(homec, this.kindergartens));
	...
	private void createOnePerson1(int i, Coord coord, Coord coordWork, String mode, String toFromPrefix) {
		...
		Leg toTrip = scenario.getPopulation().getFactory().createLeg(mode);
		plan.addLeg(toTrip);
		...
		Leg fromTrip = scenario.getPopulation().getFactory().createLeg(mode);
		plan.addLeg(fromTrip);
	...
	private void createOnePerson2(int i, Coord coord, Coord coordWork, Coord coordKg ,String mode, String toFromPrefix) {
		...
		Leg middleTrip = scenario.getPopulation().getFactory().createLeg(mode);
		plan.addLeg(middleTrip);
		
		Activity work = scenario.getPopulation().getFactory().createActivityFromCoord("work", coordWork);
		work.setEndTime(17*60*60);
		plan.addActivity(work);
 
		Leg middleTrip1 = scenario.getPopulation().getFactory().createLeg(mode);
		plan.addLeg(middleTrip1);
	...
	private void createOnePerson3(int i, Coord coord, Coord coordWork, Coord coordShop ,String mode, String toFromPrefix) {
		...
		Leg toTrip1 = scenario.getPopulation().getFactory().createLeg(mode);
		plan.addLeg(toTrip1);
		...
		Leg fromTrip1 = scenario.getPopulation().getFactory().createLeg(mode);
		plan.addLeg(fromTrip1);
	...
	public static void main(String[] args) {
		...
		BasicDemandGenerator cd = new BasicDemandGenerator();
		cd.run();
	}
	...
	private Map<String,Coord> readFacilityLocations (String fileName){
		...
		config.setDelimiterRegex("\t"); //indica che il delimitatore tra campi è \tab
		config.setCommentRegex("#"); //indica che il delimitatore dei commenti è\#
		config.setFileName(fileName); //indica al nome del file da leggere, necessario quando si istanza il metodo
