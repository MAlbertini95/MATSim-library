...
package org.matsim.Football.Demand;
...
/**
 * @author teoal on the basis of dgrether
 *
 */
public class SimpleFootballFanCreator implements FootballFanCreator {

	private SimpleFeature cbFeature;
	private SimpleFeature spnFeature;
	private Random random;
	private int fanId = 1;
	private double FootballFansPercentage = 0.25;
	private Geometry sdfGeometry;
	private double earliestArrivalTimeAtStadium = 17 * 3600;

	public SimpleFootballFanCreator(String kreisShapeFile) {
	...
	public Population createAndAddFans(Scenario sc, int numberOfFans) {
		int numberOfCbFans = (int) (numberOfFans * FootballFansPercentage);
...
//		Scenario sc2 = ScenarioUtils.createScenario(sc.getConfig());
		...
		for (int i = 0; i < numberOfCbFans; i++){
			p = pop.getFactory().createPerson(Id.create(Integer.toString(fanId) + "_" + FanFootballStrings.CB2FB, Person.class));
			pop.addPerson(p);
			fanPop.addPerson(p);
			this.fanId++;
...
//			sc2.getPopulation().addPerson(p);
		...
		for (int i = 0; i < numberOfSPNFans; i++){
			p = pop.getFactory().createPerson(Id.create(Integer.toString(fanId) + "_" + FanFootballStrings.SPN2FB, Person.class));
			pop.addPerson(p);
			fanPop.addPerson(p);
			this.fanId++;
