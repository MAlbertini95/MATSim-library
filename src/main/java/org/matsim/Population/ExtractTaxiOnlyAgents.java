...
public static void main(String[] args) {
	...
	new PopulationReader(scenario).readFile("scenarios/Car2AT/Car2ATaxiConvertedplans.xml.gz");
	double scale = 1.0;
	Population pop2 = convertPopulation(scenario.getPopulation(),scale);
	new PopulationWriter(pop2).write("scenarios/Car2AT/ATaxiOnly_plans"+scale+".xml.gz");
}
		...
		for (Plan plan : p.getPlans()){
			...
			for (PlanElement pe : plan.getPlanElements()){
				if (pe instanceof Activity){
					...
					previousLeg = (Leg) pe;
					if (previousLeg.getMode().equals("ATaxi")) write = true;
					}
				}
